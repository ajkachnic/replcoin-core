import { crypto, secp } from "./deps.ts";
import * as base58 from "./base58.ts";
import { getAddressFromPublic } from "./wallet.ts";
...
const getTransactionId = (transaction: Transaction) => {
  ...
  hash.update(txInContent);
  hash.update(txOutContent);
...
const hasDuplicates = (txIns: TxIn[]): boolean => {
  ...
  return txIns.some((txIn) => {
    if (txIn.txOutId in found) {
      return true;
    }
    found.push(txIn.txOutId);
    return false;
  });
};
...
const validateTxIn = (
  ...
  const referencedUTxOut = aUnspentTxOuts.find((uTxO) =>
  ...
  const address = base58.decode(referencedUTxOut.address);

  const validSignature = secp.verify(txIn.signature, transaction.id, address);
  if (!validSignature) {
    console.log(
...
const signTxIn = async (
  ...
  if (getAddressFromPublic(getPublicKey(privateKey)) !== referencedAddress) {
    console.log(
  ...
  const signature: string = await secp.sign(dataToSign, privateKey);
...
const getPublicKey = (aPrivateKey: Uint8Array): Uint8Array => {
  return secp.getPublicKey(aPrivateKey);
};
